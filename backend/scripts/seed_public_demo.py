...
from app.controllers.common import invalidate_public_cache
...
from app.models.user import ph
...
def main():
    ...
    with app.app_context():
        ...
        active_fairs = db.session.scalars(Fair.active_query().order_by(Fair.created_at)).all()
        while len(active_fairs) < 3:
            spec = fair_specs.pop(0)
            name, location, department, municipality, description = spec
            fair = Fair(
                nombre=name,
                slug=slugify(name),
        ...
        for index, unit in enumerate(units[:10]):
            ...
            target_fair = active_fairs[index % 3]
        ...
        for participation in db.session.scalars(select(FairParticipation).where(
            FairParticipation.fair_id.in_([fair.id for fair in active_fairs[:3]]),
        ...
        print(" | ".join(counts))
        print("Unidades productivas disponibles: 10; productos públicos: 30")
