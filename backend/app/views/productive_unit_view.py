...
from .registration_request_view import (
...
class AdminProductiveUnitCreateSchema(Schema):
    ...
    nit = fields.String(allow_none=True, validate=validate.Regexp(r"^[0-9]{5,12}$", error="El NIT debe contener entre 5 y 12 dígitos, sin guiones ni otros caracteres."))
    registro_seprec = fields.String(allow_none=True, validate=validate.Regexp(r"^[0-9]{5,12}$", error="El registro SEPREC debe contener entre 5 y 12 dígitos, sin guiones ni otros caracteres."))
    registro_pro_bolivia = fields.String(allow_none=True, validate=validate.Regexp(r"^[0-9]{5,12}$", error="El registro PRO-BOLIVIA debe contener entre 5 y 12 dígitos, sin guiones ni otros caracteres."))
    nombres_representante = fields.String(required=True, validate=representative_name_validator)
    apellido_paterno_representante = fields.String(required=True, validate=representative_name_validator)
    apellido_materno_representante = fields.String(required=True, validate=representative_name_validator)
    ...
    logo_url = fields.String(allow_none=True, validate=validate.Length(min=1, max=500))
