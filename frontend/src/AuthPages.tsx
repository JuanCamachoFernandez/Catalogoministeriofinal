...
export function LoginPage(){
  ...
  return <main className="grid min-h-screen place-items-center bg-brand-800 p-5"><form onSubmit={submit} className="card w-full max-w-md"><h1 className="text-2xl font-bold">Portal de gestión</h1><label className="mt-5 block">Usuario o Gmail<input required autoComplete="username" className="input mt-2" value={login} onChange={e=>setLogin(e.target.value)}/></label><PasswordField label="Contraseña" value={password} onChange={setPassword} autoComplete="current-password"/><button className="btn mt-6 w-full">Ingresar</button>{message&&<p className="mt-4 text-red-700">{message}</p>}<Link className="mt-4 block text-center text-brand-600" to="/catalogo">Ir al catálogo público</Link></form></main>;
}
...
export function ChangePasswordPage(){
  ...
  return <main className="grid min-h-screen place-items-center bg-brand-800 p-5"><form onSubmit={submit} className="card w-full max-w-lg"><p className="text-sm font-semibold uppercase tracking-wide text-brand-600">Primer ingreso</p><h1 className="mt-2 text-3xl font-bold">Cambie su contraseña</h1><p className="mt-3 text-stone-600">Debe reemplazar la contraseña temporal antes de acceder al sistema.</p><PasswordField label="Contraseña temporal actual" value={current} onChange={setCurrent} autoComplete="current-password"/><PasswordField label="Nueva contraseña" value={next} onChange={setNext} autoComplete="new-password"/><PasswordField label="Confirmar nueva contraseña" value={confirm} onChange={setConfirm} autoComplete="new-password"/><p className="mt-3 text-sm text-stone-600">Mínimo 10 caracteres, con mayúscula, minúscula, número y símbolo.</p>{message&&<p className="mt-4 rounded-lg bg-red-50 p-3 text-red-700">{message}</p>}<button className="btn mt-6 w-full">Guardar nueva contraseña</button><button type="button" className="mt-3 w-full text-sm text-stone-600" onClick={()=>{localStorage.clear();navigate('/gestion/login')}}>Cerrar sesión</button></form></main>;
}
...
export function DashboardPage({area}:{area:'admin'|'expositor'}){
  ...
  return <main className="min-h-screen bg-stone-100"><header className="bg-brand-800 text-white"><div className="mx-auto flex max-w-6xl items-center justify-between p-5"><strong>Catálogo Digital de Ferias</strong><button className="flex items-center gap-2" onClick={()=>{localStorage.clear();navigate('/gestion/login',{replace:true})}}><LogOut size={18}/>Cerrar sesión</button></div></header><section className="mx-auto max-w-6xl p-6"><div className="card"><div className="flex items-center gap-3 text-brand-600">{isAdmin?<ShieldCheck size={32}/>:<Store size={32}/>}<p className="font-semibold">{isAdmin?'Panel administrativo':'Panel del expositor'}</p></div><h1 className="mt-4 text-3xl font-bold">Bienvenido, {user.first_name} {user.last_name}</h1><p className="mt-2 text-stone-600">Ingresó como {user.role.replaceAll('_',' ')}.</p><p className="mt-6 rounded-xl bg-amber-50 p-4 text-amber-900">La sesión está activa. Los módulos administrativos completos todavía están en desarrollo.</p><Link to="/catalogo" className="btn mt-6">Ver catálogo público</Link></div></section></main>;
}
