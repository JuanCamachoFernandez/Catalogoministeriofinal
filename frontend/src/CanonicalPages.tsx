...
export function ProductiveUnitProfilePage() {
  ...
  const profile = useQuery({
    ...
    queryFn: () =>
      api.get<ProductiveUnit>("/productive-unit/profile").then((r) => r.data),
  });
  const sectors = useQuery({
    queryKey: ["productive-sectors", "active"],
    queryFn: () =>
  ...
  const [editing, setEditing] = useState(false),
    [draft, setDraft] = useState<Partial<ProductiveUnit>>({});
  if (profile.isLoading) return <Loading />;
  if (profile.error || !profile.data)
    return <ErrorBox message={message(profile.error)} />;
  ...
  const save = async () => {
    try {
      ...
      await api.patch("/productive-unit/profile", {
        nombre_comercial,
        razon_social,
        nit,
        registro_seprec,
      ...
      await qc.invalidateQueries({ queryKey: ["productive-unit-profile"] });
      setEditing(false);
  ...
  return (
    <section>
      ...
      <div className="page-heading">
        <div>
          <span className="eyebrow">Mi Unidad Productiva</span>
          <h1>{unit.nombre_comercial}</h1>
          <StatusBadge value={unit.estado} />
        </div>
      ...
      <div className="profile-grid">
        <article className="panel">
          {unit.logo_url ? (
            ...
            <Empty title="Sin logotipo" />
          )}
          <Field label="Actualizar logotipo">
            <input
              className="input"
              type="file"
              accept="image/*"
              onChange={async (e) => {
                ...
                const form = new FormData();
                form.append("file", file);
                await api.post("/productive-unit/logo", form);
                await qc.invalidateQueries({
                  queryKey: ["productive-unit-profile"],
                });
              }}
        ...
        <article className="panel">
          <p>
            <strong>Razón social:</strong> {unit.razon_social}
          </p>
          <p>
            <strong>Representante:</strong> {unit.nombre_representante}
          </p>
          <p>
            <strong>Contacto:</strong> {unit.telefono_whatsapp} ·{" "}
            {unit.correo_electronico}
          </p>
          <p>
            <strong>Dirección:</strong> {unit.departamento},{" "}
            {unit.direccion_fisica}
          </p>
          <p>{unit.resena_comercial}</p>
          <p>
            <strong>Sectores:</strong>{" "}
            {unit.sectores.map((s) => s.nombre).join(", ")}
          </p>
        </article>
      </div>{" "}
      {editing && (
        <Modal title="Editar perfil" onClose={() => setEditing(false)}>
          <div className="form-grid">
            {(
              [
                ...
                ["tiktok_url", "TikTok"],
              ] as [keyof ProductiveUnit, string][]
            ).map(([key, label]) => (
              <Field key={key} label={label}>
                <input
                  className="input"
          ...
          <Field label="Reseña">
            <textarea
              ...
              value={draft.resena_comercial ?? ""}
              onChange={(e) =>
                setDraft((d) => ({ ...d, resena_comercial: e.target.value }))
              }
            />
          </Field>
          <Field label="Sectores">
            <div className="check-grid">
              {sectors.data?.map((s) => (
                <label key={s.id}>
                  <input
                    type="checkbox"
                    checked={unit.sectores.some((x) => x.id === s.id)}
                    readOnly
                  />
                  {s.nombre}
                </label>
            ...
            <small>
              La gestión detallada de sectores usa el contrato específico de
              perfil.
            </small>
          </Field>
          <button className="btn" onClick={() => void save()}>
...
export function UnitSectorsPage() {
  ...
  const available = useQuery({
    ...
    queryFn: () =>
      api
        .get<{ items: ProductiveSector[] }>("/productive-sectors")
        .then((r) => r.data.items),
  });
  const own = useQuery({
    ...
    queryFn: () =>
      api
        ...
        .then((r) => r.data.sectores),
  });
  const effective = Object.keys(selected).length
    ? selected
    : Object.fromEntries(
        (own.data ?? []).map((s) => [s.id, s.detalle_otro ?? ""]),
      );
  const toggle = (sector: ProductiveSector) =>
    setSelected((current) => {
      const base = Object.keys(current).length
        ? { ...current }
        : Object.fromEntries(
            (own.data ?? []).map((s) => [s.id, s.detalle_otro ?? ""]),
          );
      if (sector.id in base) delete base[sector.id];
      else base[sector.id] = "";
      return base;
    });
  const save = async () => {
    try {
      const rows = Object.entries(effective).map(
        ([productive_sector_id, detalle_otro]) => ({
          productive_sector_id,
          detalle_otro: clean(detalle_otro),
        }),
      );
      await api.put("/productive-unit/sectors", { sectores: rows });
      setSelected({});
      await qc.invalidateQueries({ queryKey: ["productive-unit-sectors"] });
      await qc.invalidateQueries({ queryKey: ["productive-unit-profile"] });
      feedback.success(
      ...
      feedback.error("No se pudieron guardar", message(e));
    }
  };
    ...
    <section>
      <div className="page-heading">
        <div>
          ...
          <p>
            Seleccione al menos uno. Si elige “Otros”, detalle su actividad.
          </p>
        </div>
      </div>
      <div className="panel check-grid">
        {available.data?.map((s) => (
          <div key={s.id}>
            <label>
              <input
                type="checkbox"
                checked={s.id in effective}
                onChange={() => toggle(s)}
              />
              {s.nombre}
            </label>
            {s.es_otro && s.id in effective && (
              <input
                className="input"
                required
                placeholder="Detalle de Otros"
                value={effective[s.id]}
                onChange={(e) =>
                  setSelected((current) => ({
                    ...effective,
                    ...current,
      ...
      <button
        className="btn"
        disabled={!Object.keys(effective).length}
        onClick={() => void save()}
      >
        Guardar sectores
      </button>
...
export function PublicCatalogPage() {
  ...
  const active = useQuery({
    queryKey: ["public", "active-fair"],
    queryFn: () =>
      api
        .get<{ active: boolean; fair: CanonicalFair | null }>(
          "/public/fairs/active",
        )
        .then((r) => r.data),
  });
  ...
  const products = useQuery({
      queryKey: ["public", "products", q, sector, department, page],
      queryFn: () =>
        api
          .get<
            Paged<CanonicalProduct> & {
              active_catalog: boolean;
              fair?: CanonicalFair;
            }
          >("/public/products", {
            params: {
    ...
    <>
      ...
      <main className="container public-main">
        {active.isLoading ? (
          ...
          <Empty
            title="No hay una feria activa"
            description="El catálogo volverá a estar disponible cuando una feria esté publicada dentro de sus fechas."
          />
        ) : (
          <>
            <section className="public-hero">
              <div>
                <span className="eyebrow">Feria activa</span>
                <h1>{active.data.fair?.nombre}</h1>
                <p>{active.data.fair?.descripcion}</p>
                <strong>
                  {active.data.fair?.ubicacion} ·{" "}
                  {active.data.fair?.fecha_inicio} al{" "}
                  {active.data.fair?.fecha_fin}
                </strong>
              </div>
              {active.data.fair?.imagen_portada && (
                <img
                  src={assetUrl(active.data.fair.imagen_portada)}
                  alt={`Portada de ${active.data.fair.nombre}`}
                />
              )}
            </section>
            <div className="toolbar">
              <SearchField
                ...
                placeholder="Buscar productos o unidades…"
              />
              <select
                className="input"
                value={sector}
            ...
            {products.isLoading ? (
              ...
              <ErrorBox message={message(products.error)} />
            ) : data.items.length ? (
              <>
                <div className="card-grid">
                  {data.items.map((p) => (
                    <article className="catalog-card" key={p.id}>
                      ...
                      <div className="card-body">
                        ...
                        <h2>{p.nombre_comercial}</h2>
                        <p>{p.descripcion_tecnica}</p>
                        <small>{p.unidad_productiva?.nombre_comercial}</small>
                        <strong>
            ...
            {groups.length > 0 && (
                ...
                onClick={async () => {
                  try {
                    const response = await api.post<{ url: string }>(
                      "/public/whatsapp",
                      {
                        items: groups.map(([product_id, quantity]) => ({
                          product_id,
                          quantity,
                        })),
                      },
              ...
              >
                Consultar por WhatsApp ({groups.length})
              </button>
            )}
          </>
        )}
