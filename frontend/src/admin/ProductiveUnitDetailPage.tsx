...
import { message } from "./adminShared";
import { ProductiveUnitParticipationsSection } from "./ProductiveUnitParticipationsSection";
import { ProductiveUnitProductDetailPage } from "./ProductiveUnitProductDetailPage";
import { ProductiveUnitProductsSection } from "./ProductiveUnitProductsSection";
...
export function ProductiveUnitDetailPage({
  unitId,
  onBack,
  onChanged,
}: {
  ...
  if (unit.error || !unit.data) {
    return <ErrorBox message={message(unit.error ?? "Unidad no encontrada")} />;
  }

  if (selectedProduct) {
    return (
      <ProductiveUnitProductDetailPage
  ...
  return (
    <article className="admin-unit-detail-page">
      ...
      <ProductiveUnitProductsSection
        products={products}
        onSelectProduct={setSelectedProduct}
      />

      {participations.isLoading ? (
        <Loading label="Cargando participaciones..." />
      ) : participations.error ? (
        <ErrorBox message={message(participations.error)} />
      ) : (
        <ProductiveUnitParticipationsSection
