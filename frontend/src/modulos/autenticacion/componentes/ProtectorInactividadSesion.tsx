...
export function ProtectorInactividadSesion() {
  ...
  useEffect(() => {
    ...
    const check = () => {
      const status = estadoInactividad(Date.now() - lastActivityRef.current);
    ...
    const storage = (event: StorageEvent) => {
      ...
      if (estadoInactividad(Date.now() - value) === "active") {
        setLocked(false);
        setError("");
        setPassword("");
      }
  ...
  const unlock = async (event: React.FormEvent) => {
    ...
    setError("");
    try {
      await servicioAutenticacion.reauthenticate(password);
      const now = Date.now();
      lastActivityRef.current = now;
      localStorage.setItem(activityKey, String(now));
      setPassword("");
      ...
      setError(errorApi(reason, "No se pudo desbloquear la sesión."));
    } finally {
      setPending(false);
    }
  };
