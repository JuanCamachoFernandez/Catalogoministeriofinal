...
import { errorApi, type ProductiveSector } from "../../../compartido";
import { CajaError, EstadoCarga, useRetroalimentacion } from "../../../compartido/componentes";
...
export function PaginaSectoresUnidad() {
  ...
  const available = useQuery({
    ...
    queryFn: servicioUnidadProductiva.getAvailableSectors,
  });
  const own = useQuery({
    queryKey: ["productive-unit-sectors"],
    queryFn: servicioUnidadProductiva.getOwnSectors,
  });
  ...
  const save = async () => {
    ...
    try {
      ...
      feedback.error("No se pudieron guardar", errorApi(error));
    } finally {
      setSaving(false);
    }
  };

  if (available.isLoading || own.isLoading) return <EstadoCarga />;
  if (available.error || own.error) {
    return <CajaError mensaje={errorApi(available.error ?? own.error)} />;
  }
