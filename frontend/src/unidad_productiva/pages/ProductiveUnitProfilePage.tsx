...
import { apiError, assetUrl, type ProductiveUnit } from "../../api";
...
export function ProductiveUnitProfilePage() {
  ...
  const profile = useQuery({
    ...
    queryFn: productiveUnitApi.getProfile,
  });
  if (profile.isLoading) return <Loading />;
  if (profile.error || !profile.data) {
    return <ErrorBox message={apiError(profile.error)} />;
  }
  ...
  const save = async () => {
    ...
    try {
      await productiveUnitApi.updateProfile({
      ...
      feedback.error("No se pudo actualizar", apiError(error));
    }
  };
          ...
          >
            ...
            <input
              ...
              onChange={async (event) => {
                ...
                try {
                  await productiveUnitApi.uploadLogo(file);
                  await queryClient.invalidateQueries({
                    queryKey: ["productive-unit-profile"],
                  });
                  feedback.success(
