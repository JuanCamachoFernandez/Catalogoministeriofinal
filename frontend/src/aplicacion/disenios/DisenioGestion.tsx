...
import { urlRecurso } from "../../compartido";
import { useAutenticacion } from "../../modulos/autenticacion/contexto/ContextoAutenticacion";
...
> = [
  ["/admin", "Resumen", LayoutDashboard, "dashboard"],
  ["/admin/solicitudes", "Solicitudes", ClipboardList, "registration-requests"],
...
export function SelloInstitucional({ className = "" }: { className?: string }) {
  return (
    <img
...
export function DisenioGestion({
  children,
  area,
}: {
  ...
  const links = area === "admin"
    ? adminNav
        .filter(([, , , feature]) => puedeAccederFuncionAdministracion(user?.role, feature))
        .map(([to, label, icon]) => [to, label, icon] as const)
    : unitNav;
    ...
    <ProveedorRetroalimentacion>
      ...
      >
        ...
        <header className="management-header">
          ...
          <SelloInstitucional className="management-seal" />
